...
import {
  selectMobileSliderLoader,
  selectMobileSliderData,
  getMobileSliderSuccess,
} from "../Features/Slices/MobileSliderSlice";
import Splashscreen from "../Splashscreen/Splashscreen";

SwiperCore.use([Autoplay, Navigation]);

export default function MobileSlider({ initialData }) {
  const dispatch = useDispatch();
  const SliderViewData = useSelector(selectMobileSliderData);
  const isSliderLoading = useSelector(selectMobileSliderLoader);

  const [page, setPage] = useState(1);
  useEffect(() => {
    if (initialData?.result?.length > 0) {
      dispatch(getMobileSliderSuccess(initialData));
  ...
  const fetchData = () => {
    dispatch({
      type: "FETCH_MOBILE_SLIDER_VIEW_REQUEST",
      payload: {
        page: page,
        limit: 4,
      },
  ...
  return (
    ...
    >
      <Swiper
        className="swiper-slider h-[80vh]"
        centeredSlides={true}
        grabCursor={true}
        loop={true}
        mousewheel={false}
